...
-- ==========================================
-- Basic folding commands (default vim)
...
-- Additional fold shortcuts with leader
map("n", "<leader>z", "za", { desc = "Toggle fold" })
map("n", "<leader>Z", "zA", { desc = "Toggle all folds recursively" })

-- Toggle fold column
map("n", "<leader>uz", function()
	if vim.wo.foldcolumn == "0" then
		vim.wo.foldcolumn = "1"
		print("Foldcolumn: visible")
	else
		vim.wo.foldcolumn = "0"
		print("Foldcolumn: hidden")
	end
...
vim.api.nvim_create_autocmd("LspAttach", {
	...
	callback = function(ev)
		...
		map(
			...
			"gO",
			"<cmd>Telescope lsp_document_symbols<CR>",
			{ buffer = ev.buf, desc = "Document symbols outline" }
		)
